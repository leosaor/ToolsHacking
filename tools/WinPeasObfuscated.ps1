...
Write-Color "LEOSAOR TESTING" -Color Green
...
if ($FullCheck) {
  Write-Host "**Full Check Enabled.**"
}
Write-Host -BackgroundColor Red -ForegroundColor White "leosaor"

Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host "====================================||SYSTEM ||===================================="
" run the cmdlet get-computerinfo"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| WINDOWS HOTfixs"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| DriveInfo"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| AV Detect"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Windows Event Forward registry"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Creds Guard Check"
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Cached WinLogon Creds Check"
if (Test-Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon") {
  ...
  Write-Host "Or, using the 'mimi' lsadump cache"
}

Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Additonal Winlogon Creds Check"
...
if ((Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System).EnableLUA -eq 1) {
  ...
  Write-Host "u can" -ForegroundColor Yellow
}
...
if ((Get-ItemProperty HKLM:\SOFTWARE\Policies\Microsoft\Windows\Installer -ErrorAction SilentlyContinue).AlwaysInstallElevated -eq 1) {
  ...
  Write-Host "ucan " -ForegroundColor Yellow
}
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| WSUS check for http and UseWAServer = 1, if true, might be "
Write-Host "ucan" -ForegroundColor Yellow
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| STARTUP APPLICATIONS "
"Check if "
Write-Host "ucan" -ForegroundColor Yellow
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Check Token access here: ucan" -ForegroundColor yellow
Write-Host -ForegroundColor Blue "=========|| Check if you are inside the group or if you have enabled any token that can be use toucan"
Write-Host "ucan" -ForegroundColor Yellow
...
Write-Host ""
if ($TimeStamp) { TimeElapsed }
Write-Host -ForegroundColor Blue "=========|| Cloud Creds Check"
...
Write-Host "Use the 'mimi' 'dpapi cred' module with appropriate masterkey to decrypt" 
Write-Host "You can also extract many DPAPI masterkeys from memory with the mimi 'sekurlsa dpapi' module" 
