...
IF "!CurrentFolder!" NEQ "!CurrentFolder: =!" (
    ECHO leotest.bat cannot run if the current path contains spaces.
	ECHO Exiting.
    EXIT /B 1
)
...
ECHO.   [i] If '1' then you can install a .msi file with admpriv ;)
...
ECHO.   [i] Check if you are inside the Administrators group or if you have enabled any thoken
ECHO.
...
CALL :ColorLine "%E%32m[*]%E%97m SERVICE VULN"
CALL :T_Progress 1
ECHO.
